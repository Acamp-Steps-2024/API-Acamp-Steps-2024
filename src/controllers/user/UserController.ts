...
class UserController {
  static async getAllUsers(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (req.query.church) {
        ...
        allUsers = allUsers.filter((user) => {
          return normalizeString(user.church).includes(searchChurch);
        });
      }
      ...
      (res as ResponseInterface).apiSuccess({ statusCode: StatusCodes.OK, message: "Users Retrieved Successfully", data: dataResponse });

    } catch (error) {
      (res as ResponseInterface).apiFailure({ statusCode: StatusCodes.INTERNAL_SERVER_ERROR, message: "Cannot get users from database", error: error });
    }
  }
  ...
  static async getUserById(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!user) {
        (res as ResponseInterface).apiFailure({ statusCode: StatusCodes.NOT_FOUND, message: "User Not Found" });
        return;
      }

      (res as ResponseInterface).apiSuccess({ statusCode: StatusCodes.OK, message: "User Retrieved Successfully", data: user });
